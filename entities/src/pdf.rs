...
fn write_text_with_font(ops: &mut Vec<Op>, text: &str, size: f32, x: f32, y: f32, font: &PdfFontHandle) {
  let normalized: String = text.nfc().collect();
  ops.push(Op::StartTextSection);
  ops.push(Op::SetFont { font: font.clone(), size: Pt(size) });
  ops.push(Op::SetTextCursor { pos: Point::new(Mm(x), Mm(y)) });
  ops.push(Op::ShowText { items: vec![TextItem::Text(normalized)] });
...
fn draw_footer(ops: &mut Vec<Op>, font_bold: &PdfFontHandle, font_mono: &PdfFontHandle, qr_image_id: Option<&XObjectId>, pdf_id: &str, codificacion: &str) {
...
fn draw_left_sidebar(ops: &mut Vec<Op>, font_mono: &PdfFontHandle, issuer_did: &str, kid: &str, alg: &str, date: &str) {
  let lines = [
  ...
  for text in lines {
    ops.push(Op::StartTextSection);
    ops.push(Op::SetFont { font: font_mono.clone(), size: Pt(6.2) });
    ops.push(Op::SetTextMatrix { matrix: TextMatrix::TranslateRotate(Mm(x).into(), Mm(30.0).into(), 90.0) });
    ops.push(Op::ShowText { items: vec![TextItem::Text(fit_text(&text, 250).nfc().collect())] });
    ops.push(Op::EndTextSection);
...
fn draw_table_row(ops: &mut Vec<Op>, font_regular: &PdfFontHandle, y: &mut f32, a: &Asignatura) {
  let x = [CONTENT_LEFT, 86.0, 108.0, 122.0, 140.0, 160.0, PAGE_W - 12.0];
  let y_top = *y;
  let y_bot = *y - 6.0;
  ...
  write_text_with_font(ops, &format!("{:.2}", a.calificacion), 7.6, 123.0, y_top - 4.1, font_regular);
  write_text_with_font(ops, &fit_text(&a.anyo_academico, 8), 7.6, 141.0, y_top - 4.1, font_regular);
  write_text_with_font(ops, &fit_text(a.observaciones.as_deref().unwrap_or(""), 18), 7.6, 161.0, y_top - 4.1, font_regular);

  *y = y_bot;
}

fn draw_section_title(ops: &mut Vec<Op>, font_bold: &PdfFontHandle, y: &mut f32, text: &str) {
  write_text_with_font(ops, text, 11.0, CONTENT_LEFT, *y, font_bold);
  *y -= 6.5;
}

fn draw_subtitle(ops: &mut Vec<Op>, font_bold: &PdfFontHandle, y: &mut f32, text: &str) {
  write_text_with_font(ops, text, 9.2, CONTENT_LEFT, *y, font_bold);
  *y -= 4.8;
}

fn draw_label_value(ops: &mut Vec<Op>, font_bold: &PdfFontHandle, font_regular: &PdfFontHandle, y: &mut f32, label: &str, value: &str) {
...
fn ensure_space(pages: &mut Vec<PdfPage>, ops: &mut Vec<Op>, y: &mut f32, needed: f32, font_bold: &PdfFontHandle, font_mono: &PdfFontHandle, qr_image_id: Option<&XObjectId>, pdf_id: &str, codificacion: &str, issuer_did: &str, kid: &str, alg: &str, date: &str) {
  if *y - needed >= CONTENT_BOTTOM {
    return;
  }

  pages.push(PdfPage::new(Mm(PAGE_W), Mm(PAGE_H), std::mem::take(ops)));
  draw_footer(ops, font_bold, font_mono, qr_image_id, pdf_id, codificacion);
...
pub fn generate_pdf(json_str: &str, out_pdf: &str, jwt: &str, issuer_did: &str, kid: &str, alg: &str, pdf_id: &str, ts: u64) -> Result<()> {
  ...
  let one = serde_json::from_value::<DatosTituladoInner>(v.get("datos_identificativos_del_titulado").cloned().context("missing datos_identificativos_del_titulado")?)?;
  let two = serde_json::from_value::<InfoTitulacionInner>(v.get("informacion_de_la_titulacion").cloned().context("missing informacion_de_la_titulacion")?)?;
  let three = serde_json::from_value::<NivelTitulacionInner>(v.get("informacion_sobre_el_nivel_de_la_titulacion").cloned().context("missing informacion_sobre_el_nivel_de_la_titulacion")?)?;
  let four = serde_json::from_value::<ContenidoResultadosInner>(v.get("informacion_sobre_el_contenido_y_los_resultados_obtenidos").cloned().context("missing informacion_sobre_el_contenido_y_los_resultados_obtenidos")?)?;

  let codificacion = format!("{:x}", Sha256::digest(jwt.as_bytes()));
  let qr_content = "http://localhost:3000";

  let datetime = DateTime::<Utc>::from_timestamp(ts.try_into().unwrap(), 0).ok_or_else(|| anyhow!("invalid timestamp"))?;
...

  {
    ...
    dyn_img.write_to(&mut std::io::Cursor::new(&mut qr_png), image::ImageFormat::Png).context("cannot encode QR PNG")?;
  }

  let mut doc = PdfDocument::new("Certificado de Titulación");
  let mut image_warnings = Vec::<PdfWarnMsg>::new();

  let qr_image = RawImage::decode_from_bytes(&qr_png, &mut image_warnings).map_err(|e| anyhow!("cannot decode QR image: {}", e))?;
  let qr_image_id = Some(doc.add_image(&qr_image));
  let logo_image_id = RawImage::decode_from_bytes(LOGO_BYTES, &mut image_warnings).ok().map(|img| doc.add_image(&img));

  let unicode_font = try_load_unicode_font(&mut doc);
  let font_regular = unicode_font.clone().unwrap_or(PdfFontHandle::Builtin(BuiltinFont::Helvetica));
  let font_bold = unicode_font.clone().unwrap_or(PdfFontHandle::Builtin(BuiltinFont::HelveticaBold));
  let font_mono = unicode_font.clone().unwrap_or(PdfFontHandle::Builtin(BuiltinFont::Courier));

  let mut pages: Vec<PdfPage> = Vec::new();
  let mut ops: Vec<Op> = Vec::new();

  draw_footer(&mut ops, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion);
  ...
  ensure_space(&mut pages, &mut ops, &mut y, 40.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  ...
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "1.3 - Fecha de nacimiento", &one.fecha_nacimiento);
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "1.4 - Número de identificación", &one.numero_identificacion);

  ensure_space(&mut pages, &mut ops, &mut y, 28.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  draw_section_title(&mut ops, &font_bold, &mut y, "2 - Información sobre la titulación");
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "2.1 - Nombre de la titulación", &two.nombre_titulacion);
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "2.2 - Nombre de la institución", &two.nombre_institucion);

  ensure_space(&mut pages, &mut ops, &mut y, 24.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  draw_section_title(&mut ops, &font_bold, &mut y, "3 - Información sobre el nivel de la titulación");
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "3.1 - Nivel de la titulación", &format!("Nivel {} del MECES.", three.nivel_titulacion));
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "3.2 - Duración oficial del programa", &format!("{} ECTS. {} años.", three.ects_global, three.duracion_oficial));

  ensure_space(&mut pages, &mut ops, &mut y, 44.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  draw_section_title(&mut ops, &font_bold, &mut y, "4 - Información sobre el contenido y los resultados obtenidos");
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "4.1 - Modalidad de estudio", &four.modalidad);
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "4.2 - Fecha de finalización", &four.descripcion_programa.fecha_fin);

  draw_subtitle(&mut ops, &font_bold, &mut y, "Asignaturas");

  ensure_space(&mut pages, &mut ops, &mut y, 12.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  draw_table_header(&mut ops, &font_bold, &mut y);

  for a in &four.descripcion_programa.asignaturas {
    ensure_space(&mut pages, &mut ops, &mut y, 8.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  ...
  ensure_space(&mut pages, &mut ops, &mut y, 16.0, &font_bold, &font_mono, qr_image_id.as_ref(), pdf_id, &codificacion, issuer_did, kid, alg, &date);
  draw_label_value(&mut ops, &font_bold, &font_regular, &mut y, "4.3 - Calificación global del titulado", &format!("{}", four.calificacion_global));
  ...
  writer.write_all(&pdf_bytes).context("cannot save output pdf")?;
